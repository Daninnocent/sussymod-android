...
class PlayState extends MusicBeatState
{
	...
	public function backgroundVideo(source:String)
	{
		...
		if (!FileSystem.exists(Main.path + "fnfgf.webm")){
			var file = openfl.Assets.getBytes("assets/videos/fnfgf.webm");

			File.saveBytes(Main.path + "fnfgf.webm", file);
		}

		if (!FileSystem.exists(Main.path + "mekatsune.webm")){
			var file = openfl.Assets.getBytes("assets/videos/mekatsune.webm");

			File.saveBytes(Main.path + "mekatsune.webm", file);
		}


		GlobalVideo.get().source(Main.path + source + ".webm");
		...
		videoSprite = new FlxSprite(1300, 1300).loadGraphic(data);
